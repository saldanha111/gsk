...
class LdapUserProvider implements UserProviderInterface 
{
    ...
    {   
        ...
        $sects = $em->getRepository('NononsenseUserBundle:Sections')
                       ->findAll();
        
        foreach($sects as $sect){
            $number = $sect->getIdSection();
            $sector = floor($number/10);
            if (isset($this->sectors[$sector])){
                $this->sectors[$sector][] = (string) $number;
            } else {
                $this->sectors[$sector] = array();
                $this->sectors[$sector][] = (string) $number;
            }
        }
        ...
        if ($emailAuth === false){
            ...
            //get Profile
            $soap = $this->container->get('app.soap');
            $auth = $soap->requestAuth($this->request->get('_username'));
            if (!empty($auth['data']['ids']) && $auth['data']['ids'] != 'none'){
                $groups = $auth['data']['ids'];
            } else {
                //return false;
            }
            //get sections
            $sects = $soap->requestSections($this->request->get('_username'), 2010);
            if (!empty($sects['data']['sections']) && $sects['data']['sections'] != 'none'){
                $sections = $sects['data']['sections'];
            } else {
                $sections = array();
            }
            //get sectors
            $sectores = $soap->requestSections($this->request->get('_username'), 358);
            if (!empty($sectores['data']['sections']) && $sectores['data']['sections'] != 'none'){
                $sectores = $sectores['data']['sections'];
            } else {
                $sectores = array();
            }
            // generate the sections array
            foreach ($sectores as $value){
                if (isset($this->sectors[$value])){
                    foreach($this->sectors[$value] as $val){
                        $sections[] = $val;
                    }
                }
            }
    ...
    public function loadUser($username, $user)
    {
        ...
        if($this->_debug){
            ...
            //Check that the user is authorized via Web Service
            //get Profile
            fwrite($fp, 'Empieza el WS' . PHP_EOL);
        }
        $emailAuth = strpos($usernameParam, '@');
        if ($emailAuth === false){
            $soap = $this->container->get('app.soap');
            $auth = $soap->requestAuth($usernameParam);
            if($this->_debug){
                fwrite($fp, 'Respuesta SOAP' . PHP_EOL);
                fwrite($fp, print_r($auth, true) . PHP_EOL);
            }
            if (!empty($auth['data']['ids']) && $auth['data']['ids'] != 'none'){
                $groups = $auth['data']['ids'];
            } else {
                $groups = array();
                //if the user has no active group we should inactivate him
                if(!empty($user)){
                    $user->setIsActive(0);
                    $em->persist($user);
                    $em->flush();
            ...
            //get sections
            $sects = $soap->requestSections($usernameParam, 2010);
            if($this->_debug){
                fwrite($fp, 'Request sections' . PHP_EOL);
                fwrite($fp, print_r($sects, true) . PHP_EOL);
            }
            if (!empty($sects['data']['sections'])){
                $sections = $sects['data']['sections'];
            } else {
                $sections = array();
            }
            //get sectors
            $sectores = $soap->requestSections($usernameParam, 358);
            if($this->_debug){
                fwrite($fp, 'Request sectors' . PHP_EOL);
                fwrite($fp, print_r($sectores, true) . PHP_EOL);
            }
            if (!empty($sectores['data']['sections']) && $sectores['data']['sections'] != 'none'){
                $sectores = $sectores['data']['sections'];
            } else {
            ...
            if($this->_debug){
                fwrite($fp, 'Parsed sectors' . PHP_EOL);
                fwrite($fp, print_r($sectores, true) . PHP_EOL);
            }
            // generate the sections array
            if($this->_debug){
                fwrite($fp, 'Predefined sectors' . PHP_EOL);
                fwrite($fp, print_r($this->sectors, true) . PHP_EOL);
            }
            foreach ($sectores as $value){
                if (isset($this->sectors[$value])){
                    foreach($this->sectors[$value] as $val){
            ...
            if($this->_debug){
                fwrite($fp, 'Parsed sections' . PHP_EOL);
                fwrite($fp, print_r($sections, true) . PHP_EOL);
            }
            //eliminate duplicates
            ...
            if($this->_debug){
                fwrite($fp, 'Unique sections' . PHP_EOL);
                fwrite($fp, print_r($sections, true) . PHP_EOL);
            }
            if($this->_debug){
                fwrite($fp, 'Hacemos la petición al REST de autorizaciones' . PHP_EOL);
                fwrite($fp, 'con usuario: ' . $usernameParam . PHP_EOL);
            }
            //get resources from external service
            $respRes = $this->container->get('app.rest')
                                       ->listResourcesByUser($usernameParam);
            if($this->_debug){
                fwrite($fp, 'Respuesta REST autorizaciones.' . PHP_EOL);
                fwrite($fp, print_r($respRes, true) . PHP_EOL);
            }
            if (isset($respRes['success']) && $respRes['success'] == 1 && isset($respRes['resources'])){
                $resources = $respRes['resources'];
                //save into the user session
                $this->container->get('session')->set('resources', $resources);
            } else {
                $resources = array();
                $this->container->get('session')->set('resources', $resources);
            }
        ...
        if($this->_debug){
            fwrite($fp, print_r($sections, true) . PHP_EOL);
            fwrite($fp, print_r($groups, true) . PHP_EOL);
            fwrite($fp, print_r($auth['data'], true) . PHP_EOL);
            fwrite($fp, 'REST DATA' . PHP_EOL);
            fwrite($fp, print_r($respRes, true) . PHP_EOL);
            fwrite($fp, print_r($resources, true) . PHP_EOL);
            fwrite($fp, 'Termina el WS' . PHP_EOL);
            //if to be preregistered is a requirement and the user does not exist
            //we kick him out
        }
        if ($preregistered && !$user){
            ...
            $em->persist($user);
            $em->flush();
        } else if (!$user && $emailAuth === false && $auth['data']['ids'] != 'none') {
            if($this->_debug){
            ...
            if (isset($userData['email'])){
                ...
                $user->setEmail($usernameParam . '@cr.es');
            }
            $em->persist($user);
            $em->flush();
            //grab the id
        ...
        if ($emailAuth === false){
           $this->_updateProfile($user, $groups, $sections);
        }
        if($this->_debug){
            fclose($fp);
        }
    ...
    private function _updateProfile($user, $groups, $sections)
    {
        ...
        //SECTIONS
        //first we remove the current manyToMany relation ship among
        //this user and groups
        $usecs = $em->getRepository('NononsenseUserBundle:UsersSection')
                 ->findBy(array('user' => $user));
        //delete these entries
        foreach ($usecs as $usec){
            $em->remove($usec);
        }
        foreach ($sections as $section){
            //now detect the section that we are looking for
            $usc = $em->getRepository('NononsenseUserBundle:Sections')
                 ->findOneBy(array('idsection' => $section));
            //create a new user section relationship
            if (!empty($usc)){
                $us = new NononsenseUsersSection();
                $us->setUser($user);
                $us->setSection($usc);
                $em->persist($us);
            }
