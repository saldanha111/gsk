...
class ArchiveTypesController extends Controller
{
    ...
    private function saveData(Request $request, ArchiveTypes $category)
    {
        ...
        try {
            ...
            if($action){
                $this->get('utilities')->saveLogArchive($this->getUser(),$action,$comment,"type",$category->getId(),NULL,NULL,$changes);
            }

            if($actionActive){
                $this->get('utilities')->saveLogArchive($this->getUser(),$actionActive,$comment,"type",$category->getId());
            }
